...
case "${option}" in
    ...
    start)
        ...
        createUserNetwork
        if [[ $# -eq 0 ]]; then
            startDocker "$(ask_cluster_security)" "${kdc_container_name}" "${zk_container_name}" "${kafka_container_name}" "${db_type}" "${registry_container_name}" "${storm_nimbus_container_name}" "${storm_supervisor_container_name}" "${storm_ui_container_name}" "${streamline_container_name}"
        else
